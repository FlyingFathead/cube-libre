...
export const PANIC=Object.freeze({outsideSeconds:3,cooldownSeconds:30,pullSeconds:.65,holdSeconds:1.2,openSeconds:.9});
...
export function panicStatus(game) {
  ...
  return {visible:Boolean(visible),enabled:Boolean(active&&!game.panic&&!game.paused&&!game.help&&seconds===0),
    warning:Boolean(visible&&game.outside&&game.heat>0),text:seconds>0?`COOLDOWN ${seconds} s`:''};
}
