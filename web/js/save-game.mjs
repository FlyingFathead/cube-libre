import {BALANCE} from './difficulty.mjs';
...
export function validateCheckpoint(data) {
  if(!data||typeof data!=='object'||data.schema!==1||!['level','bonus','ending'].includes(data.stage))return null;
  ...
  return {schema:1,stage:data.stage,level:data.level,cells:[...data.cells],score:data.score,
...
export class CheckpointStore {
  ...
  get note() {
    ...
    return `${place} · Saved in this browser`;
  }
}
