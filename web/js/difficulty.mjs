...
export function featuresForSettings(settings=createChangeSettings(),flags={},lossMinLevel=BALANCE.lossMinLevel) {
...
export function introductionsForLevel(level,route3d=true,settings=createChangeSettings(),flags={},lossMinLevel=BALANCE.lossMinLevel) {
  return featuresForSettings(settings,flags,lossMinLevel).filter(f=>f.level===level&&(f.flag!=='route3d'||route3d)&&(!f.flag||flags[f.flag]!==false)&&(!Object.hasOwn(CHANGES,f.id)||flags.change_1!==false)).map(f=>f.state);
}

export function introductionCard(state,level,flags={},settings=createChangeSettings(),lossMinLevel=BALANCE.lossMinLevel) {
  const feature=featuresForSettings(settings,flags,lossMinLevel).find(f=>f.state===state&&f.level===level);
