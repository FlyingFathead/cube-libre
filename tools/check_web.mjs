...
const expectedComponents=files.filter(p=>/\.(?:mjs|js|css|json|svg|ogg|mp3|md)$/.test(p)&&relative(web,p)!=='version.json').map(p=>'./'+relative(web,p).replaceAll('\\','/')).sort();
assert.deepEqual(components,expectedComponents,'Every runtime component must be covered by the forced refresh');
assert.equal(readFileSync(resolve(web,'assets/PHILOSOPHY.md'),'utf8'),readFileSync(resolve(root,'docs/PHILOSOPHY.md'),'utf8'),'Run node tools/prepare_web_release.mjs after editing the philosophy');
...
for(const name of ['shutter_close','shutter_open','loss_weep','arrival_water','panic','backup_choir'])assert.ok(manifest[name],`Missing web sound: ${name}`);
