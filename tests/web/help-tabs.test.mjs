...
function buildHelp({bonus=false,connected=false,paused=false}={}) {
  ...
  const writes=[],ctx={document,game,controller:{enabled:true,connected},modalKind:null,previousPaused:false,$:id=>nodes[id],clearInput(){},syncAudio(){},focusGame(){},write:(...x)=>writes.push(x),
    createHelpTabs,createVisualOptions,releaseAssetURL,featuresForSettings,shutterGateCount,shutterInterval,BONUS_SCHEDULE,PIECES_RULES,BALANCE,release:{version:'0.23.1',upstream:{version:'0.15.79'}}};
...
test('actual Help separates keyboard, controller and visual-only options, including bonus-specific maps',()=>{
  for(const bonus of [false,true])for(const connected of [false,true]) {
    ...
    assert.deepEqual(tabs.map(t=>t.textContent),['KEYBOARD','CONTROLLER','OPTIONS']);
    ...
    assert.deepEqual(panels[2].querySelectorAll('input').map(i=>i.dataset.setting),['shake','rotation_shocks','portal_white_light']);
...
test('tab clicks and arrow/Home/End keys hide inactive panels, move focus and never advance play',()=>{
  ...
  tabs[2].click();assert.equal(panels[2].hidden,false);assert.equal(tabs[2].tabIndex,0);assert.equal(tabs[0].tabIndex,-1);
  for(const [from,key,next] of [[2,'ArrowRight',0],[0,'ArrowLeft',2],[2,'Home',0],[0,'End',2]]) {
...
test('controller navigation reaches tabs and visible options, with scrolling confined to the active panel',()=>{
  ...
  const tabs=body.querySelectorAll('[role="tab"]'),panels=body.querySelectorAll('[role="tabpanel"]');
  tabs[1].focus();navigateControllerMenu(root,{menuStep:1,scroll:0,actions:{}},document,.1);assert.equal(document.activeElement,tabs[2]);
  navigateControllerMenu(root,{menuStep:0,scroll:0,actions:{confirm:true}},document,.1);assert.equal(panels[2].hidden,false);
  navigateControllerMenu(root,{menuStep:1,scroll:1,actions:{}},document,.1);
  assert.equal(document.activeElement,panels[2].querySelector('input'));assert.equal(panels[2].scrollTop,55);assert.equal(panels[1].scrollTop,0);assert.equal(root.scrollTop,0);
  tabs[0].click();tabs[2].focus();navigateControllerMenu(root,{menuStep:1,scroll:0,actions:{}},document,.1);
